...
class User:
    ...
    def place_order(self, warehouse):
        ...
        if order:  # Check if there's anything in the cart to order
            warehouse.update_inventory(order)
            self.cart.clear_cart()  # Clear the cart after placing the order
        else:
...
warehouse.display_stock()  # To show the stock after placing the order

user1.show_cart()  # The cart should now be empty after the order

# The rest of the code remains as the example usage that shows how the classes are used.
